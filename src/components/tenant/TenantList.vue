<template>
		<div class="tenant-list">
				...
				<div class="tenant-card">
						...
						<div :class="['card-list list',item.expireTime?'is-run':'is-overdue']" v-for="item in businessesList"
						     @click.stop="onGoPage(item,'admin')"
						     :key="item.id">
								<div class="card-header">
										<strong>{{item.name}}</strong>
										<img v-if="item.versionType==='Official'" src="../../assets/images/tenant/icon-release-x2.png"
										     alt="">
										<img v-else src="../../assets/images/tenant/icon-trial.png" alt="">
										<div class="tenant-logo">
												<img v-if="item.logo" :src="item.logo" alt="" style="border-radius: 50%">
												<img v-else src="../../assets/images/tenant/icon-tenant-logo.png" style="border-radius: 50%"
												     alt="">
										</div>
								</div>
								<p style="color: #44444f">{{item.industryName}}</p>
								<div class="card-footer">
										<p>创建时间：{{$format(item.createTime,'YYYY-MM-DD')}}</p>
										<p>{{item.expireTime?('过期时间：'+$format(item.expireTime,'YYYY-MM-DD')):'已过期'}}</p>
										<div class="actions">
												<Button v-if="item.isCreator" type="default" class="see-detail"
												        @click.stop="onGoPage(item,'tenant')">详情
												</Button>
												
												<div style="display: inline-block">
														<Poptip trigger="hover" content="content" width="300">
																<Button @click.stop="onUpgrade"
																        v-if="(!item.expireTime||(item.expireTime&&item.versionType==='Official'))&&item.isCreator"
																        type="primary">续费
																</Button>
																<div class="man-phone women-phone" slot="content">
																		<img src="../../assets/images/tenant/icon-consult.png" alt="">
																		<p>想咨询续费事宜</p>
																		<p>欢迎拨打热线</p>
																		<p class="is-strong">{{contactPhone}}</p>
																</div>
														</Poptip>
												</div>
				...
				<div class="contact-button">
						<Poptip content="content" width="300">
								<Button size="large" type="primary">
										<img style="vertical-align: middle;" src="../../assets/images/tenant/icon-consult.png" alt="">
										产品咨询
								</Button>
								<div class="man-phone" slot="content">
										<img src="../../assets/images/tenant/icon-consult.png" alt="">
										<p>想了解更多内容</p>
										<p>欢迎拨打热线</p>
										<p class="is-strong">{{contactPhone}}</p>
