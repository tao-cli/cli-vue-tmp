<template>
		<div class="tenant-detail">
				<h2 class="h2">
						<span @click="$router.replace('/tenant/list')"><img :src="$backIcon" alt="">商户详情</span>
				</h2>
				<!--基本信息-->
				<div class="detail-card clear">
						<div class="left-card">
								<img :src="detailObj.logo||default_icon" class="logo" alt="logo">
								<p><strong style="font-size: 24px;">{{detailObj.name}}</strong></p>
								<p><span
												:class="detailObj.status==='running'?'is-running':'is-stop'">{{runStatusMap[detailObj.status]}}</span>
								</p>
								<p>
										<img :src="versionTypeMap[detailObj.versionType]?versionTypeMap[detailObj.versionType].icon:''"
										     alt=""
										     :title="versionTypeMap[detailObj.versionType]?versionTypeMap[detailObj.versionType].title:''">
										<img :src="authStatusMap[detailObj.authStatus]?authStatusMap[detailObj.authStatus].icon:''" alt=""
								...
								<p>
										<span class="is-active pointer"
										      @click="onUpgrade"
										      style="font-size: 14px;margin-right: 5px;">立即升级</span>
										<span class="is-active pointer"
										      @click="onChangeValid"
										      style="border-left:1px solid #E2E2EA;font-size: 14px;padding-left: 5px;">更改认证</span>
								</p>
						
						</div>
						<div class="right-card">
								<div class="fix-actions">
										<Button class="button" type="primary" @click="onUpdateTenant">编辑</Button>
										
										<Dropdown @on-click="onDropActions">
												<a href="javascript:void(0)">
														<Icon class="icon" color="#FF8C08" size="28" type="ios-more"></Icon>
												</a>
												<DropdownMenu slot="list">
														<!--2020年3月17日11:39:32暂时关闭商户入口-->
														<!--<DropdownItem class="text-left" v-if="detailObj.status==='running'" name="close">关闭商户
														</DropdownItem>-->
														<DropdownItem class="text-left" v-if="detailObj.status==='closed'" name="open">启用商户
														</DropdownItem>
														<DropdownItem class="text-left" name="delete">删除</DropdownItem>
												</DropdownMenu>
										</Dropdown>
				...
				<div class="detail-org">
						...
						<div class="org-list clear">
								<div class="left-org">
										<Tabs type="card" @on-click="onSelectNextOrg">
												<TabPane v-for="item in storeList" :key="item.id" :label="item.name"></TabPane>
										</Tabs>
								</div>
						</div>
						<div class="org-detail">
								...
								<div class="org-card">
										<div class="card-item">
												<div class="icon">
														<img src="../../assets/images/tenant/employee.png" alt="">
												</div>
												<p class="strong"><strong>{{$formatNumber(storeDetail.employeeNum,0)}}</strong>人</p>
												<p class="p">门店员工数</p>
										</div>
										<div class="card-item">
												<div class="icon">
														<img src="../../assets/images/tenant/icon-member.png" alt="">
												</div>
												<p class="strong"><strong>{{$formatNumber(storeDetail.memberNum,0)}}</strong>人</p>
												<p class="p">注册会员数</p>
										</div>
										<div class="card-item">
												<div class="icon">
														<img src="../../assets/images/tenant/icon-product.png" alt="">
												</div>
												<p class="strong"><strong>{{$formatNumber(storeDetail.shopsNum,0)}}</strong>件</p>
												<p class="p">商品数</p>
										</div>
								</div>
						</div>
				...
				<div class="detail-logs">
						...
						<div class="clear">
								<h3 style="float: left;font-size: 18px;">操作记录</h3>
								<i-input style="float: right;" class="w250" v-model="pageData.searchKey" placeholder="操作人 / 操作内容 检索"
								         @on-enter="onSearch"
								         @on-click="onSearch"
								         search></i-input>
						</div>
						<div class="table-content">
								<BaseTable :list="tableList" :col="tableCol"
								           :pageData="pageData"
								           @on-pageSize="onPageSize"
								           @on-pageChange="onPageChange"></BaseTable>
						</div>
				</div>
				<Modal v-model="updateModalStatus"
				       title="编辑商户"
				       :mask-closable="false"
				...
				>
						<div class="body">
								<TenantEdit style="width: 100%;" ref="tenantEdit" v-if="updateModalStatus"
								            @onCloseModal="onCloseUpdateModal"></TenantEdit>
						</div>
						
						<div class="footer" slot="footer">
								<Button type="primary" ghost @click="onCloseUpdateModal">取消</Button>
								<Button type="primary" @click="onConfirmUpdateModal">确认</Button>
						</div>
				</Modal>
		</div>
</template>
