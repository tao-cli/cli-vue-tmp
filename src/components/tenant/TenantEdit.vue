				...
				<div class="tenant-content">
						<div class="form-body">
								<div class="avatar-item">
										...
										<div class="image-position" v-show="tenantData.logo">
								...
								>
										<FormItem label="商户类型" prop="storeType">
												<i-select :disabled="$route.name==='tenant-detail'" v-model="tenantData.storeType"
												          placeholder="请选择商户类型">
										...
										<FormItem label="商户名称" prop="name">
												<i-input v-model.trim="tenantData.name" placeholder="请输入商户名称" :maxlength="30"></i-input>
										</FormItem>
										...
										<FormItem label="商户地址" prop="addressList">
												<Cascader placeholder="请选择省 / 市 / 区" :data="cascadeCitys"
												          v-model="tenantData.addressList"></Cascader>
										</FormItem>
										<FormItem>
												<div class="address-detail">
														<i-input v-model="tenantData.address.addressDetail" :maxlength="50"
														         class="pr-80"
														         placeholder="请填写具体地址" @on-change="onSearchMap"></i-input>
														<span class="address-span is-active" @click="onReSearch">搜索地图</span>
														<div class="address-map">
																<BMap ref="bMap"
																      :geoLocation="provinceCityArea"
										...
										<Button v-if="!isUpdate" size="large" long type="primary" @click="onCreateUpdateTenant">创建</Button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getIndustryList() {
				this.$ajaxGet('/user-server/platform/industry/listSimple')
					.then(res => {
						if (res && res.code === 10000) {
			...
			getTenant() {
				...
				this.$ajaxGet('/user-server/platform/store/update/detail', {id: tenantId})
					.then(res => {
						if (res && res.code === 10000) {
							const json = res.data || {};
							let addressList = [json.address.province, json.address.city, json.address.area].filter(item => item);
			...
			// 完成创建之后跳转到后台
			finishCreate(orgId) {
				this.$ajaxGet('/user-server/auth/switchOrg?orgId=' + orgId || '')
					.then(res => {
						if (res && res.code === 10000) {
