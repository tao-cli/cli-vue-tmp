   ...
   .city-box{
     ...
     ul{
       ...
       li{
         ...
         >i{display: inline-block; height: 12px; width: 12px; border: solid 1px #ff8c08;}
       }
       li.isActived{
        //  &:hover{background: #ff8c08;}
     ...
     i.area-checkbox-icon{
       display: inline-block; width:12px; height: 12px; border:solid 2px #ff8c08;
     }
   }
 </style>
 <style lang="scss">
   .city-box{
     ...
     .ivu-icon-md-checkbox{color:#ff8c08}
     .ivu-checkbox-group{
      ...
      //  label.isActived{background: #ff8c08;color:#fff}
     }
   }
 </style>
<template>
  <div >
    <section class="city-box">
      <aside>
        <h3>省份</h3>
        <ul>
          <li v-for="item in provinces" :key="item.id+Math.random()" 
            @click="toggleProvince(item.id)" :class="{'isActived':item.id.substr(0,2)==curProvince,'isDisable':disableProvince.includes(item.id)}">
            <span @click.stop="disableProvince.includes(item.id)?null:checkItem(item,'province')">
              ...
              <span class="ivu-checkbox-indeterminate" style="margin-left:9px;vertical-align:middle;" 
                v-if="hasMinateArr(item.id)&&!provinceChoices.includes(item.id)">
                <span class="ivu-checkbox-inner"></span>
              </span>
              <Icon type="md-checkbox" v-else-if="provinceChoices.includes(item.id)"  />
              <Icon type="ios-square-outline" v-else />
            </span>
            <label>{{ item.name }} </label>
            <span><Icon type="ios-arrow-forward" style="color:#ccc" /></span>
          </li>
        </ul>
      ...
      <aside>
        <h3>城市</h3>
        <ul>
          <li v-for="item in citys.filter(item=>item.id.substring(0,2)==curProvince)" :key="item.id+Math.random()"
            ...
            <span @click.stop="disableCity.includes(item.id)||disableProvince.includes(item.parent+'0000')?null:checkItem(item,'city')">
              <Icon type="md-checkbox" v-if="cityChoices.includes(item.id)"  />
              <Icon type="ios-square-outline" v-else />
            </span>
            <label>{{item.name }} </label>
          </li>
    ...
    <div style="margin-top:20px; text-align:right;">
      <Button @click="cancelCity" class="mr-10">取消</Button>
      <Button type="primary" @click="confirmCity" :disabled="!provinceChoices.length&&!cityChoices.length">确定</Button>
    </div>
  </div>
</template>
<script>
