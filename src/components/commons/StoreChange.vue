...
<template>
  <div>
    <Form :model="storeInfor" :label-width="120" ref="formValidate" :rules="ruleValidate" >
      <FormItem label="操作礼品：">
        <section v-if="goods" class="goods-item">
          <img :src="goods.pictureUrl" alt="商品图片">
          <div>
            <h3 :title="goods.goodsName">{{goods.goodsName}}</h3>
      ...
      <FormItem  class="store-btn-box">
        ...
        <Button type="primary" @click="confirmStore" :loading="btnLoading">提交</Button>
      </FormItem>
    </Form>
  </div>
</template>
