  ...
  <i-col span="24" class="cardDescView">
    ...
    <div  class="cardDescCont" v-else >
      <ul>
        ...
        <li>
          <h3>适用范围</h3>
          <div class="prodList" v-if="infor.couponType=='CASH'">
            {{infor.applyScopeType?infor.applyScopeType==2?'全品类':'':'-'}}
            <div v-if="convention">
              <div v-for="(value) in curConvention" :key="value.title">
                <span style="color:#999;">{{ value.title }}</span>
                <p><Tag v-for="(item,index) in value['list']" type="border" :key="index" color="primary">{{index>4?item.title+'等':item.title}}</Tag></p>
              </div>
            </div>
          </div>
          <div v-else> - </div>
        ...
        <li>
          ...
          <div v-show="infor.useTimeType">
            <div>{{infor.useTimeType==4?'不限时间':infor.useTimeType==1
            ...
            ?'领券当日起 '+(infor.useDays||1)+' 天内可用':'领券次日起'+(infor.useDays||1)+"天内可用"}}</div>              
          </div>
        </li>
      </ul>
    </div>
