...
<template>
  <div class="power-table">
    ...
    <Table  :style="{minWidth:'200px',width:'100%',overflow:'hidden','overflowY':'auto'}" :height="maxHeight"
      :columns="columns" :data="datas" border :span-method="!notSpan?handleSpan:null" 
      :show-header="!$_.isEmpty(multipleCol)" @on-selection-change="(selection)=>$emit('on-selectTable',selection)">
      <div slot='header' v-if="$_.isEmpty(multipleCol)">
        <SelfTitle :title="defaultCols" >
          <span v-for="item in columns" :key="item.key" :style="{flex:'0 0 '+(item.width)+'px'}">
            <label><i v-if="item.isRequired" style="color:#f05">* </i>{{ item.title }}</label>
            <Input type="text"  size="small" v-if="item.isEdit&&item.type=='text'" 
              @on-change="(e)=>titleChange(e.target.value,item.key,item.onlyNum)" :maxlength="item.maxlength" />
            <InputNumber  size="small" v-if="item.isEdit&&item.type=='number'" :value="item.value*1||null" style="width:100%"
              @on-change="(value)=>titleChange(value,item.key)" :max="item.max" :min="0" />
          </span>
        </SelfTitle>
      </div>
    </Table>
  ...
  methods:{
    initData(){
     ...
     // 1. 根据度量值构造表格行数，n*m
      ...
      console.log(this.dimension,this.counts,this.columns)
      // 2.根据维度和度量构建表格
    ...
    handleSpan(row){ 
      ...
      if(this.dimension[columnIndex]){
        ...
        if(columnIndex==0){
          ...
          let number1 = data1.number,divide1 = data1.divide;
          ...
          let number2 = data2.number,divide2 = data2.divide;
    ...
    evalDimension(number,divideName){
      ...
      return cond.split(',').filter(item=>item).join("||");
    },
    ...
    initTable(list,skus){
      ...
      for(var i=0;i<list.length;i++){
        ...
        if(list.length==1){
          list[i].specContents.map(pitem=>{
            aItem = {...aItem,aname:pitem.specContent,id:[pitem.specContentId]};
            curList.push(aItem)
          })
        }else{
          var index = i;
          list[i].specContents.map(pitem=>{
            aItem = {...aItem,aname:pitem.specContent,id:[].concat(pitem.specContentId)};
            for(var i=index+1;i<list.length;i++){
              aItem = {...aItem,...list[i],bname:list[i].specName}
              if(list.length==2){
                var mIds = aItem.id;  // 用于记录前两级id值
                list[i].specContents.map(pitem=>{
                  aItem = {...aItem,bname:pitem.specContent,id:mIds.concat(pitem.specContentId)};
                  curList.push(aItem)
                })
              }else{
                var flag = i;
                ...
                list[i].specContents.map(pitem=>{
                  aItem = {...aItem,bname:pitem.specContent,id:tIds.concat(pitem.specContentId)};
                  for(var i=flag+1;i<list.length;i++){
                    aItem = {...aItem,...list[i],cname:list[i].specName}
                    if(list.length==3){
                      let curIds = aItem.id; // 用于记录前三级id值
                      list[i].specContents.map(pitem=>{
                        aItem = {...aItem,cname:pitem.specContent,id:curIds.concat(pitem.specContentId)};
                        curList.push(aItem)
                      })
                    }
                  }
      ...
      console.log('skus:',skus,'selected:',this.selected,'filter:',this.filterSkus)
      // 根据前面封装好的数据标记的id数组，去sku 里匹配具体的条目
      curList = curList.map(item=>{
        let cId = item.id.join(","),curItem=JSON.parse(JSON.stringify(item));
        let cSku = skus.filter(sitem=>sitem.specContentIds&&sitem.specContentIds.join(",")===cId)
        if(cSku&&cSku.length){
          curItem = {...item,...cSku[0]}
          if(curItem.skuId){
        ...
        // 如果是多选，且有选中的，则匹配显示
        if(this.selected.includes(curItem.skuId)||this.selected.includes(curItem.skuId*1)){
...
<style lang="scss">
  .power-table{
    width:100%;
    .ivu-table-cell{overflow: inherit !important;}
  }
</style>