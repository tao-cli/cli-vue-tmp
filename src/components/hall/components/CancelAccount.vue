<template>
		<div class="account-cancel">
				<h2 class="h2">
						<span @click="goAccountPage"><img :src="$backIcon" alt="">注销账号</span>
				</h2>
				<div class="cancel-form">
						<Form ref="cancelAccount"
						      class="form-body"
						      :model="cancelObj"
						      :rules="cancelObjValid"
						>
								...
								<!--验证码-->
								<FormItem v-if="setup===0"
								          style="margin-bottom: 10px;margin-top: 38px;"
								          prop="messageCode">
										<div>
												<i-input
																type="text" :maxlength="6" v-model="cancelObj.messageCode"
								...
								<!--风险提示-->
								<div class="notify" v-if="setup===1">
										<strong class="notify-strong">您的账号 {{cancelObj.areaCode}} {{cancelObj.phone}} 注销后有以下风险</strong>
										<ul class="ul-item">
										...
										<Checkbox v-model="cancelObj.isCheckBox">
												已阅读并同意 <a href="javascript:alert('稍后呈现')"><span class="is-active">《用户注销协议》</span></a>
										</Checkbox>
								</div>
						</Form>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.cancelObj.phone = phone;
			this.cancelObj.areaCode = areaCode;
		...
		methods: {
			...
			checkNext() {
				...
				this.$ajaxPost('/user-server/auth/checkMessageCode', {phone, messageCode})
					.then(res => {
						if (res&&res.code === 10000) {
							this.setup = 1;
						}
			...
			cancelAccount() {
				...
				this.$ajaxPost('/user-server/auth/confirmCancellation', {id})
					.then(res => {
						if (res&&res.code === 10000) {
