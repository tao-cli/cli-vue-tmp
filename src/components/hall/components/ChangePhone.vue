...
<template>
		<div class="account-changePhone">
				<h2 class="h2">
						<span @click="goAccountPage"><img :src="$backIcon" alt="">更换手机号</span>
				</h2>
				<div class="changePhone-form">
						<Form ref="changePhone"
						      class="form-body"
						      :model="accountData"
						      :rules="accountObjValid">
								<FormItem v-if="setup===0" label="MOS 账号">
										<div class="mos-item">
								...
								<!--手机账号-->
								<FormItem v-else style="margin-top: 38px;" prop="phone">
												...
												<Dropdown @on-click="onClickArea">
														...
														<DropdownMenu slot="list">
																<DropdownItem v-for="item in countryAreaList" :key="item.prefix"
																              :name="item.prefix">{{item.prefix+ item.name}}
																</DropdownItem>
														</DropdownMenu>
												</Dropdown>
										
										</div>
										<i-input type="text" :maxlength="11"
										         class="change-phone-input"
										         v-model="accountData.phone" placeholder="请输入注册时的手机号">
										</i-input>
								</FormItem>
								<FormItem v-if="setup===0" label="密码" prop="password">
										<i-input v-model="accountData.password"
										         type="password"
										         :maxlength="16" placeholder="请输入您的密码"></i-input>
								</FormItem>
								
								<!--验证码-->
								<FormItem v-show="setup===1" style="margin-bottom: 10px;margin-top: 38px;"
								          prop="messageCode">
										<div>
												<i-input
						...
						<Button style="height: 48px;margin-top: 60px;" long type="primary" @click="onNextSetup">
								{{setup?'确认修改':'下一步'}}
						</Button>
				</div>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.accountData.phone = phone;
			this.accountData.areaCode = areaCode;
		},
		computed: {},
		data() {
