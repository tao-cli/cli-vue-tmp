<template>
    <div class="container">
        <i-col span="8" class="low">
            <div>{{ lowName }}</div>
            <div class="member-name row-gap">{{ item.beforeLevelName }}</div>
        </i-col>
        <i-col span="4" class="icon" :class="(item.levelChangeType === 1) ? 'up-level' : 'down-level'">
            <i  class="icon iconfont iconTreeUnfold-copy"  title="" ></i>
        </i-col>
        <i-col span="8" class="high">
            <div>{{ highName }}</div>
            <div class="member-name row-gap">{{ item.afterLevelName }}</div>
        </i-col>
    </div>
</template>
